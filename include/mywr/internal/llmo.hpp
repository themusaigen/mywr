/**
  ...
  low-level, platform-aware memory manipulation functions. It offers safe and
  efficient methods for reading, writing, copying, filling, and comparing memory
  regions. The library incorporates error handling using `tl::expected` and
  optional memory protection adjustments. Key features include:
...
/**
  @brief This namespace provides low-level, platform-aware memory manipulation
  functions. It offers safe and efficient methods for reading, writing,
  copying, and filling memory regions, incorporating error handling and
  optional memory protection adjustments. The library uses `tl::expected` for
  error handling, providing a more robust alternative to exceptions. The
  functions are designed to be exception-safe and minimize the risk of
  unexpected behavior due to memory access violations.
 */
namespace mywr::llmo {
/**
...
/**
  @brief Flushes the instruction cache for a specified memory region.

  @details This function flushes the instruction cache for the memory region
  specified by `addr` and `size`.  This ensures that the CPU fetches the most
  ...
  @param addr The starting address of the memory region to flush.
  @param size The size of the memory region in bytes.
  @return `true` if the instruction cache was successfully flushed or if the
  operation is not supported on the current platform. `false` indicates an error
  during the flush operation on Windows.
  @note The `[[maybe_unused]]` attribute indicates that the parameters might not
...
/**
  ...
  improve performance if the memory is already readable, but requires the caller
  to ensure the memory is readable.
  @return A Result object containing the read value (Ok) or an llmo_error (Err).
...
/**
  ...
  improve performance if the memory is already writable, but requires the caller
  to ensure the memory is writable.
  ...
  @note The CPU cache is flushed after the write operation to ensure the data is
...
/**
  ...
  @param dest The destination memory address. Must be a valid address and
  writable.
...
/**
  ...
  unprotects the memory region before writing, handling potential errors such as
  invalid addresses and protection changes. The CPU cache is flushed after the
  fill operation.
...
address is invalid, an error is returned. If a memory access error occurs during
the comparison, it will return the error indicated.
  ...
  @return An `tl::expected<int, llmo_error>` object. The value is the result of
    ...
    during the comparison. Possible errors include:
          - llmo_error::InvalidAddressError: Either `buf0` or `buf1` is invalid.
 */
