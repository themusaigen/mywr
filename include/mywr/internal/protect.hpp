/**
  ...
  memory protection.  It allows for temporarily changing memory protection flags
  (e.g., read, write, execute) and ensures that the original protection is
  restored even in the presence of exceptions.  This is crucial for operations
  that require direct memory manipulation but must prevent unexpected behavior
  due to incorrect access permissions.
...
/**
  ...
  in the presence of exceptions. It's designed for scenarios where direct memory
  manipulation is necessary but requires careful management of memory access
  permissions.
...
/**
  ...
  @details This function sets the memory protection flags for a region of memory
  starting at the given address and extending for the specified size. It
  ...
  @return The memory protection flags after the attempt to set them.  May differ
  from `new_protect` if the change failed partially or entirely.  Returns
  `protection::None` if the address is invalid or if an error occurs.
 */
...
/**
  ...
  @details This function checks the memory protection flags at the given address
    to determine if the memory is readable.
  ...
  @note The function is declared as `noexcept` indicating it will not throw
    exceptions.
 */
inline auto is_readable(const address& address) noexcept -> bool {
...
/**
  ...
  @details This function checks the memory protection flags at the given address
    to determine if the memory is writable.
  ...
  @note The function is declared as `noexcept` indicating it will not throw
    exceptions.
 */
inline auto is_writeable(const address& address) noexcept -> bool {
...
/**
  ...
  @details This function checks the memory protection flags at the given address
    to determine if the memory is executable.
  ...
  @note The function is declared as `noexcept` indicating it will not throw
    exceptions.
 */
inline auto is_executable(const address& address) noexcept -> bool {
...
/**
  ...
  protection flags of a region of memory.  It ensures that the memory protection
  is restored to its original state when the object goes out of scope, even in
...
public:
  ...
  /**
    ...
    @return `true` if the memory protection was successfully changed; otherwise,
    `false`.
   */
