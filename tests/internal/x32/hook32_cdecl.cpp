...
namespace {
auto MYWR_CDECL pod_cdecl(int lhs, int rhs) -> int {
  return lhs + rhs;
}

auto MYWR_CDECL nonpod_cdecl() -> NonPod {
  return {1, 1, 1};
}

auto MYWR_CDECL nonpod_cdecl_with_initializers(int param0,
                                               int param1,
                                               int param2) -> NonPod {
...
TEST(Hook, CdeclPod) {
  ...
  hook<decltype(&pod_cdecl)> pod_hook;
  pod_hook.target(&pod_cdecl);
  pod_hook.redirect(
  ...
  EXPECT_EQ(pod_cdecl(param_1, param_2), expected_result);
}

TEST(Hook, CdeclNonPod) {
  constexpr auto expected_result{
      NonPod{0, 0, 0}
  };

  hook<decltype(&nonpod_cdecl)> nonpod_hook;
  nonpod_hook.target(&nonpod_cdecl);
  ...
  EXPECT_FALSE(result) << "Failed to install hook, error: "
                       << static_cast<int>(result.value());
  EXPECT_EQ(nonpod_cdecl(), expected_result);
}

TEST(Hook, CdeclNonPodWithInitializers) {
  ...
  hook<decltype(&nonpod_cdecl_with_initializers)> nonpod_hook;
  nonpod_hook.target(&nonpod_cdecl_with_initializers);
  ...
  EXPECT_FALSE(result) << "Failed to install hook, error: "
                       << static_cast<int>(result.value());
  EXPECT_EQ(nonpod_cdecl_with_initializers(param_1, param_2, param_3),
            expected_result);
}
